...
def populate():
    ...
    # Level 2
    sp2 = add_sp(story, 2, sp1,  "You rush into the escape pods but there is one hungy zombie in your way",  "zombie_1", "Go go to the escape pods.",10, "mid", None)
    sp3 = add_sp(story, 3, sp1,  "The study place is full of zombies!", "space_study", "Go go to the study place. It looks safe...", 10, "mid", None)
    sp4 = add_sp(story, 4, sp1,  "You close the gates, send the transmit signal and wait there drinking hot chocolate, in five days a rescue group finds and saves you!", "space_command_station", "Go to the transmission station to broadcast a help signal", 10, "end", "good")

    # Level 3
    sp5 = add_sp(story, 5, sp2,  "Zombies have a really cold sense of humor...", "zombie_3", "Tell him a joke!", 10, "end", "bad")
    sp6 = add_sp(story, 6, sp2,  "Ok you can run but so can the Zombie!", "zombie_4", "Try to run!", 10, "mid", None)
    sp7 = add_sp(story, 7, sp2,  "Ok... even though your aim sucks because the only thing you do is spend your life writing code, you managed somehow to cripple it and now it is slower", "zombie_5", "Try to kill it by throwing stuff to it!", 10, "mid", None)
    ...
    sp11 = add_sp(story, 11, sp6,  "You are in the escape pod with a preset route to earth! you are saved!", "escape_pods", "Close the Gate!", 10,"end", "bad")
    ...
    sp14 = add_sp(story, 14, sp8,  "You cant find a cure for the zombies because you are a software engineer remeber???", "lab", "On your way around you find the biology lab", 5, "end", "good")
